...
//    }
    ...
    @Test
    public void testInTimespanExclusiveOutside() {
        Instant start = Instant.parse("2016-02-17T10:30:00Z");
        Instant end = Instant.parse("2016-02-17T10:59:00Z");
        List<Tweet> result = Filter.inTimespan(Arrays.asList(tweet1, tweet2), new Timespan(start, end));
        assertTrue("no tweets should be inside this narrow span", result.isEmpty());
    }
    ...
    public void testContainingNoMatch() {
        List<Tweet> result = Filter.containing(Arrays.asList(tweet1, tweet2), Arrays.asList("nothing"));
